...
lineCounts <- read.csv("ImportantlineCounts.csv")
vocabCounts <- read.csv("ImportantvocabCounts.csv")
commentCounts <- read.csv("ImportantcommentCounts.csv")
...
ggplot(counts,aes(x=Lines,y=Comments)) + 
  geom_point(aes(color=Vocabulary))  + 
  geom_text(data=counts[counts$Comments>25,],aes(label=File))
